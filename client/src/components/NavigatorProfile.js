...
export default function NavigatorProfile() {
  ...
  useEffect(() => {
    ...
    if(selectedTab.label === "Purchases"){
      if(finished && !finished.hasOwnProperty("error")){
        setCategory(finished.map(e => {return <SelectProfileUser title={e.title} img={e.images[0].url} price={e.price}/>}))
      }else{
        setCategory(<h1 className="text-center">{selectedTab.label} Empty</h1>)
      }
    }
  ...
  return (
    ...
    <div className="w-[30rem] h-[25rem] flex flex-col rounded-lg border-4 border-primary-700/100 font-medium">
      ...
      <Link to={`/${selectedTab.label}`} className="w-full text-center no-underline text-black text-xl font-medium bg-primary-700 mt-auto rounded-lg">Go to {selectedTab.label}</Link>
    </div>
      </>
  );
}
